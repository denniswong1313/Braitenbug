...
sensorA_eqsL = '''
dvL/dt = (0.04/ms/mV)*vL**2 + (5/ms)*vL + (140*mV/ms) - uL + iLA :  volt
duL/dt = a*(b*vL - uL): volt / second
...
sensorA_eqsR = '''
dvR/dt = (0.04/ms/mV)*vR**2 + (5/ms)*vR + (140*mV/ms) - uR + iRA :  volt
duR/dt = a*(b*vR - uR): volt / second
...
#                           ****** Alpha Bug ******
    ...
    srA_pred.iRA = I1 / (sqrt(((srA.x-bug1.x)**2+(srA.y-bug1.y)**2)))**4 * mV/ms
    slA_pred.iLA = I1 / (sqrt(((slA.x-bug1.x)**2+(slA.y-bug1.y)**2)))**4 * mV/ms
...
def update_plot():
    global foodx, foody, bug1_plot, bug2_plot, bugA_plot, food_plot, sr1_plot, sl1_plot, srA_plot, slA_plot
    bug1_plot[0].remove()
    bugA_plot[0].remove()
    food_plot[0].remove()
    sr1_plot[0].remove()
